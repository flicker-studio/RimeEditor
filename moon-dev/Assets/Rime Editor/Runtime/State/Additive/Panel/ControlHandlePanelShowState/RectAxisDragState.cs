...
namespace LevelEditor
{
    public class RectAxisDragState : AdditiveState
    {
        ...
        public RectAxisDragState(Information Information, MotionCallBack motionCallBack) : base(Information, motionCallBack)
        {
            StateInit();
        }
        ...
        private void UpdateRect()
        {
            ...
            var moveDir = m_currentMouseWorldPosition - m_originMouseWorldPosition;

            if (moveDir.magnitude == 0) return;

            for (var index = 0; index < TargetObjs.Count; index++)
            {
                var     scaleDir = Vector3.one;
                ...
                var     positionOffsetProject = Vector3.zero;
                ...
                if (scaleDir == GetRectRect.transform.up || scaleDir == -GetRectRect.transform.up) rate = rate.NewX(1);

                if (scaleDir == GetRectRect.transform.right || scaleDir == -GetRectRect.transform.right) rate = rate.NewY(1);
                ...
                if (GetUseGrid)
                {
                    ...
                    var tempPosition = new Vector3(GetCellHalfHalfSize * Mathf.RoundToInt(newPosition.x / GetCellHalfHalfSize),
        ...
        private void InitDragType()
        {
            ...
            if (GetUI.GetControlHandlePanel.GetRectTopRightCornerInput) m_rectDragType = RECTDRAGTYPE.TopRightCorner;
        }
