...
namespace Frame.StaticExtensions.StaticClassMethod
{
    public static class ColliderList
    {
        ...
        {
            var colliderListGroup = new List<List<Collider2D>>();
            
            foreach (var collider in targetCollider) colliderListGroup.Add(collider.CheckColliderConnectivity(scale, layerMask));
        ...
        private class Collider2DListEqualityComparer : IEqualityComparer<List<Collider2D>>
        {
            ...
            public int GetHashCode(List<Collider2D> obj)
            {
                ...
                foreach (var o in obj.OrderBy(c => c.GetHashCode())) hash ^= o.GetHashCode();
