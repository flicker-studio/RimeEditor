    ...
    public class CopySlicer : SliceCommand
    {
        ...
        private void CutSlice(SLICEDIR slicedir, List<Collider2D> targetColliderList, List<Collider2D> oriColliderList)
        {
            ...
            foreach (var collider in tempList)
            {
                ...
                SlicedHull slicedHull = collider.gameObject.Slice(pos, rot * Vector3.up);
                if (slicedHull == null) continue;
                
                var obj = Object.Instantiate(m_slicerInformation.GetProductPrefab);
                ...
                if (Object.Instantiate(collider.gameObject /*, m_slicerInformation.GetProductPrefab*/)
            ...
            foreach (var collider in tempList)
            {
                if (!targetColliderList.Contains(collider)) continue;
                //TODO:Analysis!
                //  if (Object.Instantiate(collider.gameObject, m_slicerInformation.GetProductPrefab)) continue;
        ...
        private List<Collider2D> CheckBox(string tag)
        {
            List<Collider2D> overlapColliderList = m_slicerInformation.GetTransform.position.ToVector2()
                                                                      .OverlapRotatedBox(m_slicerInformation.GetDetectionRange
                                                                                       , m_slicerInformation.GetTransform.rotation.eulerAngles.z)
                                                                      .ToList();
        ...
        {
            var        shared    = target.GetComponent<MeshRenderer>().sharedMaterials;
